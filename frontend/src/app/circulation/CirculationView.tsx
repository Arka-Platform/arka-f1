...
import {
  useCirculationActivePayload,
  useCirculationDndRegistration,
  useCirculationQuickActions,
} from "../../components/Layout/CirculationDndContext";
...
import {
  loadCirculationFromSupabase,
  ownerKeyForBook,
  type CirculationOwner,
} from "./loadCirculationData";
import {
  avatarUrlForUserId,
  mediaCountForBook,
...
export default function CirculationView() {
  ...
  const [activeIndexByRow, setActiveIndexByRow] = useState<
      ...
      try {
        if (target === "wishlist") {
          ...
          showToastWithAction(
            ...
            () => {
              void wishlistApi
                .removeFromWishlist(user.id!, payload.bookId)
                .finally(() => {
                  window.dispatchEvent(new Event("wishlistUpdated"));
                });
            },
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings
      .filter((l) =>
        genreFilter ? sectionLabelForListing(l) === genreFilter : true,
      )
      .filter((l) => {
        if (!q) return true;
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books
      .filter((b) =>
        genreFilter ? sectionLabelForBook(b) === genreFilter : true,
      )
      .filter((b) => {
        if (!q) return true;
  ...
  const sections = useMemo(() => {
    if (kind === "listings") {
      ...
      const out = Array.from(map.entries()).map(([label, items]) => {
        ...
        const groups = Array.from(byBook.entries()).map(
          ([bookId, listings]) => ({
            bookId,
            listings,
  ...
  const activePayload = useMemo(() => {
    ...
    const row =
      sections.rows.find((r: any) => r.key === rowKey) ?? sections.rows[0];
    if (!row) return null;
  ...
  const cardPropsListing = useCallback(
    (
      ...
      const ownerAvatarUrl =
        firstOwner?.avatarUrl ?? avatarUrlForUserId(rep.ownerId);
  ...
  const updateActiveFromScroll = useCallback((rowKey: string) => {
    ...
    setActiveIndexByRow((prev) =>
      prev[rowKey] === bestIdx ? prev : { ...prev, [rowKey]: bestIdx },
    );
  }, []);

  const advanceToIndex = useCallback((rowKey: string, idx: number) => {
    const row = rowRefs.current[rowKey];
    if (!row) return;
    const cards = row.querySelectorAll<HTMLElement>(`[data-circ-card='true']`);
    const el = cards[idx];
    ...
    return (
      <div className={styles.page}>
        <div className={styles.pageInner}>
          ...
          <section
            aria-busy="true"
            aria-label="Loading circulation from Supabase"
          >
      ...
      <div className={styles.page}>
        <div className={styles.heroHeader}>
          ...
          <p className={styles.heroSubhead}>
            Browse what’s trending in the community. Pick what you want, pass
            what you don’t.
          </p>
        ...
        <div className={styles.controls} aria-label="Sort and filter">
          <div className={styles.filtersRow}>
            ...
            >
              ...
              {kind === "listings" ? (
                <option value="requests_desc">Most requested</option>
              ) : null}
        ...
        <p className={styles.emptyText}>
          No books in circulation yet. List a book on Exchange or check back
  ...
  return (
    <div className={styles.page}>
      <div className={styles.heroHeader}>
        <h1 className={styles.heroH1}>Circulation</h1>
        <p className={styles.heroSubhead}>
          Browse what’s trending in the community. Pick what you want, pass what
          you don’t.
        </p>
            ...
            <div
              ...
              onScroll={() => {
                ...
                window.requestAnimationFrame(() =>
                  updateActiveFromScroll(row.key),
                );
              }}
              onPointerDown={() => {
                if (!isMobile) return;
                ...
                (
                  ...
                  wrapCard(
                    <CirculationBookCard
                      ...
                      selected={
                        selected?.kind === "book" &&
                        selected.bookId === group.bookId
                      }
                      onSelect={() =>
                        handleSelect({ kind: "book", bookId: group.bookId })
                      }
                      passed={group.listings.every((l) =>
                        passedIds.has(l.listingId),
                      )}
                      active={
                        isMobile &&
                        activeRowKey === row.key &&
                        (activeIndexByRow[row.key] ?? 0) === idx
                      }
                      onPass={() => {
                        if (group.listings.length <= 1)
                          handlePassId(group.rep.listingId);
                        else
                          handleSelect({ kind: "book", bookId: group.bookId });
                      }}
                    />,
                ...
                (row.items as BookResponse[]).map((book, idx) =>
                  wrapCard(
                    <CirculationBookCard
                      ...
                      selected={
                        selected?.kind === "book" && selected.bookId === book.id
                      }
                      onSelect={() =>
                        handleSelect({ kind: "book", bookId: book.id })
                      }
